...
contract NFTPoolMintAndBurn is CCIPReceiver, OwnerIsCreator {
    ...
    event NFTMinted(uint256 tokenId,address newOwner);
    ...
    function burnAndSendNFT(
        ...
        address receiver) external returns(bytes32) {
            // tranfer wnft to this pool for burn
            wnft.transferFrom(msg.sender,address(this),tokenId);
            // burn wnft before sent
            wnft.burn(tokenId);

            // construct data to be sent
            bytes memory data = abi.encode(tokenId,newOwner);

            bytes32 messageId = sendMessagePayLINK(chainSelector,receiver,data);
            return messageId;
    }
        ...
        override
    {
        ...
        wnft.mintMyTokenBySpecifyTokenId(newOwner,tokenId);
